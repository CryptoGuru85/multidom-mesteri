...
def upload_project_picture(instance, filename):
    return "project/{profile}/{filename}".format(profile=instance.profile, filename=filename)
...
class User(AbstractBaseUser):
    email = Email(verbose_name='email address', max_length=255, unique=True,)
...
class Profile(models.Model):
    ...
    profile_picture = models.ImageField(upload_to=upload_profile_picture, blank=True, null=False)
...
class Project(models.Model):
    ...
    profile_picture = models.ImageField(upload_to=upload_project_picture, blank=True, null=False)
